...
#'
#' @description
#'  `SMARTTR`.  This package allows for the user-friendly pre-processing of segmentation data generated from ImageJ to a be compatible with the `wholebrain` package to generate
...
#'
#' A [SMARTTR::slice] object will contain all the data related to registration, segmentation for each channel, and cell counts for a particular image.
...
#'
#' A [SMARTTR::mouse] object is an object that will store multiple slice objects (and therefore all the information in it),
...
#' An [SMARTTR::experiment]An experiment object consists of a list of processed mouse objects with raw data from slices omitted, and experimental attributes stored as a list.
