...
Installing `wholebrain` is quite finicky. The most updated instructions on installing `wholebrain` for Windows is found [here](https://matiasandina.github.io/wholebrain_install/) . 
...
> Note that if you've changed computers and you find the location of your mouse folder has changed in some way, e.g. different drive mapping letters on Windows or different OS, you can adjust for this using the `reset_mouse_root()` function. You may also want to reset the output folder for that mouse just by modifying the mouse `output_path` attribute.

```{r}
# Change to the correct drive letter
mouse_325 <- reset_mouse_root(mouse_325, input_path = "X:\Holly Hunsberger\Sundowning_images\Cohort4_2month\325")
...
we will pass this filter to`SMARTR::register()` so the function knows which `brain.threshold` to use. 
...
Note that currently, this method is dependent on the naming conventions of the ImageJ macro used to create and save the txt file. In the future, if there are multiple different ImageJ segmentation macros with different naming conventions being used in the lab, we just need to standardize the macro to fit adhere to the current naming conventions. Alternatively, you could write multiple custom import functions. 

```{r}
mouse_325 <- import_segmentation_ij(mouse_325,
                                 slice_ID = '1_1',
                                 hemisphere = NULL,
...
When you run the `exclude_anatomy` function, it will automatically omit the regions for each hemisphere contained  in these lists. The commands below print the default excluded regions for the left and right hemispheres.
...
SMART::glassbrain2(mouse_325$cell_table$cfos, jitter = TRUE)
```
...
We can choose to overwrite the existing hippocampal data stored in the `normalized_counts` dataframe with the hippocampal data split dorsal and ventrally by setting the `merge` parameter to `TRUE`. I recommend this option, as all future analysis functions are designed to operate on this data. 
...
For now we will load a saved example experiment object named `sundowning` that already contains many mouse objects. *Please note* that the data in this object is fabricated. It is simply mouse_325's data with added noise. This is because we are still currently in the process of generating all the segmentation data for the sundowning project. Additionally, a random number of regions have been deleted across the mice to simulate differences in omitted regions in slices registered across mice.

```{r, eval = TRUE}
# Load the presaved data
load("V:\\Michelle Jin\\Sundowning\\PipelineDemo\\Experiment_example\\experiment_demo_saved.Rdata")

# Print the names of the mice stores in the sundowning object
print(names(sundowning$mice))
...
```{r, eval = TRUE}
sundowning <- combine_norm_cell_counts(sundowning, by = c('group', 'sex', 'age'))

```

`by` is a special parameter that will allow up to take advantage of the many mouse attributes that we have recorded during creation of a mouse object. It is a vector of the mouse attributes we will use to split our dataset into subgroups for comparison. In the example above, we use the `group`, `sex`, and `age` mouse attributes to compare `AD` and `control` groups and the `male` and `female` mice that are 2 months old. Sometimes, certain attributes like `drug` may not apply to your experiment, so `by` should only include the variables that you intend on using for group comparisons during analysis to avoid cluttering your combined dataframe. For consistency in the functions used to filter out subgroups, the values of the attributes will all be converted to strings.
...
```{r, eval = TRUE}
sundowning <- find_outlier_counts(sundowning, by = c("group", "sex", "age"), n_sd = 2, remove = TRUE, log = TRUE)
...
```{r, eval = TRUE}
sundowning <- enough_mice_per_group(sundowning, by = c("group", "sex", "age"), min_n = 5, remove = TRUE, log = TRUE)
```
...
```{r}
sundowning <- get_percent_colabel(sundowning, by = c("group", "sex", "age"), channel = "eyfp", save_table = TRUE, individual = FALSE)
```

Calculate percent colabel for individual mice and save it to the output folder

```{r}
sundowning <- get_percent_colabel(sundowning, by = c("group", "sex", "age"), channel = "eyfp", save_table = TRUE, individual = TRUE)
```
...
But first, let's do the same thing for the female controls.
...
## 11.3 Permute pairwise region correlation differences between groups
...
It also allows you to specify the number of shuffles, the random seed number (for figure replication), as well as a multiple comparisons adjustment (the previous adjustment from `get_correlations()` is not redundantly applied). Note that when the multiple comparison's adjustment is applied, this will change the p-value that is plotted in a volcano plot. 
...
The `alpha` parameters specifies at which threshold a node is included in the networks based on p-values of pairwise-region correlation coefficients. 
...
```{r}
plot_percent_colabel(sundowning,
                     channel = "eyfp",
                     color_mapping = "sex",
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\Colabeled_over_eyfp_plot_by_sex_group.svg){width=80%}
...
Plotting using just one attribute is also possible. The `color` aesthetic takes precedence over the `pattern` aesthetics, so just omit the `pattern_mapping` parameter, and it will take the default value of `NULL`. Below we'll only use the `color_mapping` parameter and set it to `group` to plot a separate bar for each experimental group.
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\Colabeled_over_eyfp_plot_by_group.png){width=80%}
...
```
![Example heatmap of the cfos channel](C:\Users\Jinmich\Documents\SMARTR\tutorial\heatmap_Female_AD_cfos.png){width=70%} 

## 12.3 Plot permutation analysis results (volcano plots & parallel coordinate plots)

A volcano plot can show us the the pairwise regions whose correlations are significantly different between analysis groups. The colored dots represent the region pairs that are significantly different between the groups, and have pearson correlation coefficient differences that are greater than +/- 1. 
...
volcano_plot(sundowning,
             ...
             channels = c("cfos", "eyfp", "colabel"),
             title = "Female AD vs female CTRL",
             ylim = c(0,4))
```


![Volcano plot of just the cfos channel](C:\Users\Jinmich\Documents\SMARTR\tutorial\volcano_plot_Female_AD vs female CTRL_cfos.png){width=70%}



A parallel coordinate plot allows us to see which actual region pairs are different between our two analysis groups and in which direction their correlation coefficients change. Only region pairs from the volcano plot that are above the alpha level and have correlation coefficient differences greater than absolute one are included in this graph. 
...
```
![parallel coordinate plot of just the cfos channel](C:\Users\Jinmich\Documents\SMARTR\tutorial\female_AD_vs_female_control_parallel_coordinate_plot_cfos_.png){width=70%}

## 12.4 Plot network graphs

The `plot_networks()` functions will automatically plot the output of `create_networks()`. You must specify the name of the network you want to plot. There a some customizable features such as setting the edge color (taken as a hexadecimal string) for positively correlated connections. The default color is "firebrick". Below we will plot the network for the cfos channel in only female AD mice. 
...
```
![AD female network graph for the cfos channel](C:\Users\Jinmich\Documents\SMARTR\tutorial\network_female_AD_cfos.png){width=70%}
```{r}
plot_networks(sundowning,
              network_name = "female_control",
              channels = "cfos",
...
```
![Control female network graph for cfos channel](C:\Users\Jinmich\Documents\SMARTR\tutorial\network_female_control_cfos.png){width=70%}


## 12.5 Plot network summary statistics

### Degree distributions

After we have plotted our network graphs, it is useful to also plot the summary statistics and measurements of node centrality within our networks. 

Degree is a measurement of how many nodes each node is connection to. We can plot a stacked histogram of degree distributions of a network in a stacked histogram using the function `plot_degree_distribution()`. Remember that we were able to summarize multiple networks using the function `SMARTR::summarise_networks()`, so we can actually plot both networks on one graph. Check out the example below for plotting the `female_AD` group and the `female_control` networks.

If you want to change the legend label of your groups, map the corresponding `network_name` to the label you want using the `labels` parameter. Below is an example of mapping the `female_AD` group and `female_control` group to the legend labels "Female AD" and "Female CTRL" respectively.


```{r,  }
plot_degree_distributions(sundowning,
                          color_palettes = "reds",
                          channels = "cfos",
                          title = "Degree distribution",
                          labels = c("female_AD" = "Female AD",
                                     "female_control" = "Female CTRL"),
                          height = 10,
                          width = 10,
                          xlim = c(0, 5),
                          ylim = c(0, 40),
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\networks_degree_distributions_cfos.png){width=60%}


### Mean degree

We can also plot the mean degree between the networks on a bar graph. Here the parameter `rev_x_scale` allows us the change the order of our groups on the barplot. In this case, setting it to `TRUE` will keep the control analysis group on the left. 

```{r,  }
plot_mean_degree(sundowning,
                 color_palettes = c("reds"), 
                 channels = c("cfos"),
                 title = "",
                 labels = c("female_AD" = "Female AD",
                            "female_control" = "Female CTRL"),
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\networks_mean_degree_cfos.png){width=60%}

Also, Keep in mind that you may need to tweak the `ylim` of your graph to ensure that all the data can fit within the axes limits. If one bar seems to not plot, try changing this parameter first.

### Clustering coefficient

The clustering coefficient is a measurement of how many neighboring nodes to a given node are mutually connected, i.e. the strength of local clusters. We can use the function `plot_mean_clust_coeff()` to compare the average clustering coefficient for each network.


```{r,  }
plot_mean_clust_coeff(sundowning, 
                      color_palettes = c("reds"),
                      channels = "cfos",
                      title = "",
                      rev_x_scale = TRUE,
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\networks_mean_clust_coeff_cfos.png){width=60%}

### Mean global efficiency

Efficiency is a measure of the average number of nodes necessary to cross to get from any node to another. Efficiency is the inverse of the average shortest path. To make an analogy, think about the game "6 degrees of Kevin Bacon". 

![Kevin Bacon in his natural element](https://hips.hearstapps.com/hmg-prod.s3.amazonaws.com/images/kevin-bacon-index-1621021995.jpg){width=50%}



If one person in our lab is 6 degrees away from Kevin, while another is only 3 degrees away (distant cousins, I guess). Then the efficiency in those two people's connection to Kevin Bacon is 1/average_degree = 1/4.5. 

```{r,  }
plot_mean_global_effic(sundowning,
                       channels = "cfos",
                       color_palettes = "reds",
                       labels = c("female_AD" = "Female AD",
                            "female_control" = "Female CTRL"),
                       title = "Mean global efficiency",
                       ylim = c(0, 0.3),
                       height = 10, 
                       width = 10,
                       rev_x_scale = TRUE)
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\networks_mean_global_efficiency_cfos.png){width=60%}

### Mean betweenness centrality

A related concept is betweenness centrality. This measures how many shortest paths pass through a given node, and it is one indication of how important or central a given node is.

```{r,  }
plot_mean_between_centrality(sundowning,
                             color_palettes = "reds",
                             channels = "cfos",
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\networks_mean_betweenness_centrality_cfos.png){width=60%}

### Region degree

For a single network, We can also examine which individual regions have the highest degree. This will give use an indication of how central some of these regions are in their network and presumably related to how functionally important they are. We can do this by plotting region degrees in descending order.

The example below plots the "female_AD" network region degrees. 

```{r,  }
plot_degree_regions(sundowning,
                    network = "female_AD", 
                    channels = "cfos",
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\networks_degree_per_region_cfos.png){width=100%}

### Region betweenness

Similarly to degree, we can plot the the betweenness centrality for individual regions in descending order.

```{r,  }
plot_betweenness_regions(sundowning,
                         network = "female_AD",
                         channels = "cfos",
                         colors = "#be0000",
                         title = "Betweenness",
...
![](C:\Users\Jinmich\Documents\SMARTR\tutorial\networks_betweenness_per_region_cfos.png){width=100%}
...
We can also automatically loop through the registration process all the slices in a given mouse object. The script below is an example for how to process `mouse_325.`It will loop through each slice, setting the correct ID and hemisphere parameters, and allow for interactive adjustment of the `brain.threshold` and registration. 


```{r}
# Autolooping script to do set a brain threshold and do registrations
# Use this script when you are trying to register all of the slices in a mouse object.
# First go to the mouse folder on the server and load the mouse object.
# Set the current slice you want to start processing. This script will loop through the remaining slices.
# This script will first interactively ask you to try setting different brain thresholds prior to registration each slice.
# I recommend a brain threshold setting between 6-12. Anything above this may be too sensitive.


#_____________ Set the user-defined variables below _________________

# Enter the name of the mouse you are processing as a string
mouse_name <- "mouse_325"

# Enter the number of the slice where you left off/want to start
curr_slice <- 1


#______ End of user defined variables. Highlight and run the code below to start the registration loop _________________

# Load library
library(SMARTR)

# Set current mouse to the user specified name
eval((parse(text=paste0("mouse <- ", mouse_name))))

# Process the slice names for looping
slices <- names(mouse$slices)
rights_hemi <- grep("_right", slices)
lefts_hemi <- grep("_left", slices)

# remove hemispheres from slice names
slice_ID <- gsub("_right", "", slices)
slice_ID <- gsub("_left", "", slice_ID)


# Set brain threshold and register
for (s in curr_slice:length(slices)){
  if(s %in% rights_hemi){
    hemi <- "right"
  } else if (s %in% lefts_hemi){
    hemi <- "left"
  } else {
    hemi <- NULL
  }

  # Adjust the brain filter
  filter <- adjust_brain_outline(mouse$slices[[slices[s]]])

  ## Print information about slice before the registration
  coordinate <- attr(mouse$slices[[slices[s]]],"info")$coordinate
  message(paste0("\n\nYour current slice is: ", slice_ID[s],
                 "\n The coordinate is:", coordinate,
                 "\nThe hemisphere is: ", ifelse(is.null(hemi), "both", hemi)))

  # Interactive registration
  mouse <- register(mouse,
                        slice_ID = slice_ID[s],
                        hemisphere = hemi,
                        filter = filter)
  
  # Automatically update the mouse object
  eval((parse(text=paste0(mouse_name, " <- mouse"))))
}


#_____________ Run the code below to periodically save your work  _________________

# I recommend doing this after every registration
eval((parse(text=paste0("save_mouse(", mouse_name, ", timestamp = TRUE)"))))
...
## 13.5 Looping through cleaning up cell counts
TODO

## 13.6 Looping through adding mice
TODO
