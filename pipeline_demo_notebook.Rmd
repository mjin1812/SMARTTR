...
# mouse_info$input_path    <- 'Y:/Holly Hunsberger/Sundowning_images/Cohort4_2month/325/325_2_5'
...
mouse_325 <-SMARTR::register(mouse_325, 
                      slice_ID = "2_5",
                      hemisphere = NULL,
                      filter = filter)
...
To save time, we will load a dataset that has been pre-registered:
...
# Print default regions excluded list
...
For right hemisphere slice object, we want to 1) exclude the contralateral hemisphere, 2) clean up cell counts that map outside of the brain contours, 3) exclude cell counts from layer 1 of the cortex, and 4) manually specify regions we want to exclude. 
...
Let's do the same thing for the full slice, except we set exclude_other_hemisphere to FALSE.
...
mouse_325 <- exclude_anatomy(mouse_325, 
                        ...
                        exclude_regions = c("VS"),
                        exclude_layer_1 = TRUE, 
                        exclude_other_hemisphere = FALSE)
...
```{r}
mouse_325 <- normalize_cell_counts(mouse_325, combine_hemispheres = TRUE, simplify_regions = TRUE)

```


## 9. TODO: automatically generate DV Hippocampus datasets with user specified coordinates

## 10. TODO: Create experiment object that aggregates information from multiple mice
