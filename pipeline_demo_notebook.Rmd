...
For now we will load a saved example experiment object named `sundowning`. *Please note* that the data in this object is fabricated. It is simply mouse_325's data with added noise. This is because we are still currently in the process of generating all the segmentation data for the sundowning project. 
...
It also allows you to specify the number of shuffles, the random seed number (for figure replication), as well as a multiple comparisons adjustment (the previous adjustment from `get_correlations()` is not redundantly applied). Not that when the multiple comparison's adjustment is applied, this will change the p-value that is plotted in a volcano plot. 
...
sundowning <- correlation_diff_permutation(sundowning,
                                           correlation_list_name_1 = "male_AD",
                                           correlation_list_name_2 = "male_control",
                                           n_shuffles = 1000,
                                           seed = 5,
                                           alpha = 0.005,
                                           ylim = c(0, 4),
...
volcano_plot(sundowning,
             permutation_comparison = "male_AD_vs_male_control",
             channels = c("cfos", "eyfp"),
             title = "Male AD vs Male CTRL",
             ylim = c(0,4))
```
