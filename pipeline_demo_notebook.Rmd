...
This is an R Notebook that allows for an interactive walkthrough of the wholebrain pipeline for the lab. The base of this pipeline is a package in development with the working title, `SMARTR`, a self-referential play on the previous package I developed as an extension to `wholebrain` called `SMART`. 
...
slice_info <- attr(slice_S1_9, 'info')

# Change slice attributes to reflect your slice and experiment
slice_info$slice_ID            <- '1_9'
slice_info$coordinate          <- -3.2
...
slice_info$registration_path   <- 'Y:\\Holly Hunsberger\\Sundowning_images\\Cohort 3_2month\\255\\255_S1_9\\MAX_255_1_9.tif'  
...
attr(slice_S1_9, 'info') <- slice_info
...
mouse_255 <- add_slice(mouse_255, slice_S1_9)
...
mouse_255$slices$`1_9_left`

```

### Now we are ready to register this image!
...
##
m <- mouse_58
m <- register(m, slice_ID = "AD_M_S1_7" )
