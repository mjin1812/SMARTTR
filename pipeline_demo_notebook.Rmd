...
An experiment object will primarily store the processed information from multiple `mouse` objects. Some experimental attributes will auto populate based on mouse attributes. For example, if multiple mice are added to an experiment with the `drug` attribute values `ketamine` or `saline`, the experiment attribute `drug_groups` will be a vector with the values `ketamine` and `saline`. 
...
There are also different ways to calculate the data. If the parameter `individual` is set to `TRUE`, then the counts for each individual mice are preserved.  If `individual` is set to `FALSE` then individual percentage colabelled cells are collapsed into an analysis group average. The `by` parameter is used to determine the analysis groups. There is an additional `roi` parameter which allows for the additional option of filtering for a specific brain region. Below are examples of different ways to use the function:
...
```{r}
sundowning <- get_percent_colabel(sundowning, by = c("group", "sex", "age"), channel = "eyfp", roi = "dDG", save_table = TRUE, individual = FALSE)
```
...
The first thing we may want to plot is percentage of colabelled counts over a given channel. 

This plot is designed to take up to 2 "mapping" variables to display on the barplot. 
...
Efficiency is a measure of the average number of nodes necessary to cross to get from any node to another. Efficiency is the inverse of the average shortest path. To make an analogy, think about the game "6 degrees of Kevin Bacon". One person in our lab is 6 degrees away from Kevin, while another is only 3 degrees away (distant cousins, I guess). The our efficiency in our connection to Kevin Bacon is 1/average_degree = 1/4.5. 
