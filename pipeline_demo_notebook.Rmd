...
```{r}
sundowning <- combine_norm_cell_counts(sundowning, by = c('groups', 'sex'))
...
# k <- 3
# 
#   load(paste0("V:/Michelle Jin/Sundowning/PipelineDemo/325/false_AD_mouse_", k, ".RDATA"))
  m <- normalize_cell_counts(m, combine_hemispheres = TRUE, simplify_regions = TRUE)
  m <- split_hipp_DV(m, AP_coord = -2.7, combine_hemispheres = TRUE, simplify_regions = TRUE, merge = TRUE)
...
for (k in 1:4){
   ...
   sundowning$mice[[k]]$normalized_counts$cfos[sundowning$mice[[k]]$normalized_counts$cfos$acronym=="AAA",]
...
```{r}
e <- enough_mice_per_group(e, by = "group", min_n = 3)
```
