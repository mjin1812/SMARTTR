...
Therefore, if you are processing any z-stack images, they need to be flattened and channels need to be collapsed into a single image used for the purpose of registration. This step needs to be completed prior to using SMARTR for registration. We detail how to do this in using our pre-processing [article](./articles/Segmentation.html)
...
XII.  Functions for automated visualization of the analysis functions.
...
# Add the path to the location of your example downloaded mouse object
...
```{r}
mouse_733 <- split_hipp_DV(mouse_733, AP_coord = -2.7, combine_hemispheres = TRUE, simplify_regions = TRUE, merge = FALSE)
print(mouse_733$hipp_DV_normalized_counts)
```
...
```{r}
mouse_733 <- split_hipp_DV(mouse_733, AP_coord = -2.7, combine_hemispheres = TRUE, simplify_regions = TRUE, merge = TRUE)
```
...
my_experiment <- experiment(experiment_name = "Learned Helplessness",
                            ...
                            output_path = "V:\\Michelle Jin\\path_to_output_folder")
...
For now we will load a saved example experiment object named `learned_helplessness` that already contains many mouse objects. 

```{r}
# Load the presaved data ## TODO EDIT THIS
